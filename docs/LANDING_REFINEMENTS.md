...
- The shared header measures 68 px on desktop and 80 px on mobile, excluding native safe-area padding. Its app icon, wordmark, theme control, and Sign In button enter with an 850 ms upward fade, staggered by 120 ms using the hero's easing. Reduced-motion settings show the final state immediately. Only the contents animate, preserving the header's layout and combined home link. Sign In includes a login icon; the wordmark can shrink on narrow screens to accommodate it.
